...
def generate_report():
    ...
    total_size_mb = sum([a.get('file_size_mb', 0)
                        for a in analyses if 'error' not in a])
    total_rows = sum([a.get('shape', (0, 0))[0]
                     for a in analyses if 'error' not in a])
    total_columns = sum([a.get('shape', (0, 0))[1]
                        for a in analyses if 'error' not in a])
    ...
    report_lines.append(
        f"• Successfully analyzed files: {successful_analyses}")
    ...
    file_sizes = [(a['file_name'], a.get('file_size_mb', 0))
                  for a in analyses if 'error' not in a]
    ...
    row_counts = [(a['file_name'], a.get('shape', (0, 0))[0])
                  for a in analyses if 'error' not in a]
    ...
    if total_size_mb > 100:
        ...
        report_lines.append(
            "   Consider compression optimization for large datasets")
        report_lines.append("")
    ...
    large_files = [a for a in analyses if 'error' not in a and a.get('shape', (0, 0))[
        0] > 100000]
    if large_files:
