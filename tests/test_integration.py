...
from exolife.data.utils import get_data_source, load_config
...
class TestDataIntegration:
    ...
    def test_fetch_manager_integration(
        self,
        mock_write,
        mock_download,
        mock_source_configs,
    ...
    @pytest.mark.integration
    def test_hz_calculation_integration(
        self, sample_dataframe, mock_hz_config, test_settings
    ):
...
class TestPipelineIntegration:
    ...
    def test_dag_yaml_loading_and_execution(self, temp_dir, dag_yaml_config):
        ...
        assert all(r.success for r in results.values() if hasattr(r, "success"))

    @pytest.mark.integration
    @patch("exolife.pipeline.executor.FetchManager")
    def test_real_pipeline_task_execution(
        self, mock_fetch_manager_class, mock_source_configs, test_settings
    ):
        ...
        # Test fetch task
        fetch_task = TaskNode("fetch_test", "fetch", config={"source": "test_source"})
        ...
        validate_task = TaskNode(
            "validate_test", "validate", config={"check_completeness": True}
        )
...
class TestEndToEndIntegration:
    ...
    @patch("exolife.data.fetch.fetchers.http_fetcher.stream_download")
    @patch("exolife.data.fetch.fetchers.http_fetcher.write_generic")
    def test_complete_exolife_workflow(
        self,
        mock_write,
        mock_download,
        mock_source_configs,
        ...
        with patch("exolife.data.utils.settings", test_settings):
            with patch("exolife.data.fetch.manager.settings", test_settings):
                ...
                assert all(
                    ...
                    for col in ["pl_name", "hz_inner", "hz_outer"]
                )
    ...
    def test_settings_integration_across_modules(self, temp_dir):
        ...
        # Create settings
        settings = Settings(root_dir=temp_dir, max_workers=8, log_level="DEBUG")
        ...
        with patch("exolife.data.utils.settings", settings):
            with patch("exolife.data.fetch.manager.settings", settings):
                ...
                load_config()  # Uses settings.root_dir for config path
